...
def employee_dashboard(
    ...
    require_portal(user, PortalScope.employee)
    records = accessible_records(db, user, PortalScope.employee)
    return templates.TemplateResponse(
        request=request,
        name="employee_dashboard.html",
        context=render_context(user, PortalScope.employee, records),
    )
...
def employee_ai_ask(
    ...
    payload = ask_portal_ai(user, request, PortalScope.employee, db)
...
def employee_upload_document(
    ...
    document = create_document_record(db, user=user, portal_scope=PortalScope.employee, request=request)
    return JSONResponse(
        {
